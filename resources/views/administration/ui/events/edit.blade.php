                    ...
                    <div class="col-md-9">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="form-title" name="title"
                                placeholder="Saisissez le titre d'événement" value="{{ $event->title }}">
                            <span class="text-danger error-text title_error"></span>
                        </div>
                        <div class="mb-3">
                            <label class="content">Contenu d'événement </label>
                            <textarea class="ckeditor form-control" id="event-desc" name="content" rows="40">
                                {!! $event->content !!}
                            </textarea>
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="image_holder mb-2" style="max-width: 250px;">
                            <img src="" alt="" class="img-thumbnail" id="image-previewer"
                                data-ijabo-default-img="{{ $event->image
                        ...
                        <div class="mb-3">
                            <div>
                                <label class="row">
                                    <span
                                        class="col {{ $event->approved ? 'text-success' : 'text-danger' }}">{{ $event->approved ? 'Evénement Approuvé' : 'Evén non Approuvé' }}</span>
                                    <span class="col-auto">
                                        <label class="form-check form-check-single form-switch">
                                            <input class="form-check-input" {{ $event->approved ? 'checked' : '' }}
                        ...
                        <div class="mb-3">
                            <div>
                                <label class="row">
                                    <span
                                        class="col {{ $event->readable ? 'text-success' : 'text-danger' }}">{{ $event->readable ? 'Evénement Lisible' : 'Presentation' }}</span>
                                    <span class="col-auto">
                                        <label class="form-check form-check-single form-switch">
                                            <input class="form-check-input" {{ $event->readable ? 'checked' : '' }}
                                                type="checkbox" name="readable">
                                            <span class="text-danger error-text approved_error"></span>
                                        </label>
                                    </span>
                                </label>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.code === 1) {
                        ...
                        tinymce.get('event-desc').setContent(response.content);
                        ...
                        // Mettez à jour l'état de la case à cocher et le label
                        var checkbox = $('input[name="readable"]');
                        var statusLabel = $('.col');
                        checkbox.prop('checked', response.readable);
                        statusLabel.text(response.approved ?'Evénement Lisible' : 'Presentation');
                        statusLabel.attr('class', 'col ' + (response.readable ? 'text-success' :
                            'text-danger'));
